<template>
  <div class="store_manager">
    ...
    <table>
      <tr>
        <th>ID:</th>
        <td><input type="text" v-model="login_id">
        <p v-if="error" class="error">IDを入力してください</p></td>
    ...
    <div class="password_card" v-if="password_card">
      ...
      <p>必ずメモしてください</p>
      <button class="close_button" @click="password_card = false">閉じる</button>
    </div>
  </div>
</template>
<script>
  ...
  methods:{
    make_password(){
      if(this.login_id == ""){
        ...
        axios.post("https://rese-booking.herokuapp.com/api/manage/storeManager/create",{
